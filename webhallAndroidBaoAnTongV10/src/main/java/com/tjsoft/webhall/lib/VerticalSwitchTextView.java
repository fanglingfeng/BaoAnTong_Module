...
public class VerticalSwitchTextView extends TextView implements View.OnClickListener {
	...
	public VerticalSwitchTextView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		int attrId1 = context.getResources().getIdentifier("switchDuaration", "attr", context.getPackageName());
		int attrId2 = context.getResources().getIdentifier("idleDuaration", "attr", context.getPackageName());
		int attrId3 = context.getResources().getIdentifier("switchOrientation", "attr", context.getPackageName());
		int attrId4 = context.getResources().getIdentifier("alignment", "attr", context.getPackageName());
		TypedArray array = context.obtainStyledAttributes(attrs, new int[] { attrId1, attrId2, attrId3, attrId4 });
		try {
			switchDuaration = array.getInt(0, DEFAULT_SWITCH_DURATION);
			idleDuaration = array.getInt(1, DEFAULT_IDLE_DURATION);
			switchOrientation = array.getInt(2, 0);
			alignment = array.getInt(3, TEXT_ALIGN_CENTER);
		} finally {
			array.recycle();
		}
		init();
	...
	private void init() {
		...
		ellipsis = getContext().getString(MSFWResource.getResourseIdByName(mContext, "string", "ellipsis"));
