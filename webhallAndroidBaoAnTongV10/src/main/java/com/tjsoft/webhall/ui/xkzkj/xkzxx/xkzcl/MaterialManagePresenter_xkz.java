...
public class MaterialManagePresenter_xkz extends BasePresenter implements MaterialManageContract_xkz.Presenter {
    @NonNull
    private MaterialManageContract_xkz.View view;
    @NonNull
    ...
    public MaterialManagePresenter_xkz(@NonNull MaterialManageContract_xkz.View mView) {
        this.view = mView;
    ...
    public MaterialManagePresenter_xkz(@NonNull MaterialManageContract_xkz.View mView, Context context) {
        this.view = mView;
...
//        this.STATUS = activity.getIntent().getIntExtra("STATUS", -1);
        certcode = activity.getIntent().getStringExtra("certcode");
...
//        }
        ...
        if (!TextUtils.isEmpty(certcode)) {
            atts = Constants.materialXkz.get(certcode);
            if (null == atts) {
                atts = new ArrayList<>();
...
//        }
    ...
    public void getOneCertInfo(String token, String usercode, String certcode, Context context) {
        ...
        Disposable disposable = zhengwuRepository.getOneCertInfo(xkzkjSendbean, context)
                        ...
                        if (value != null) {
                            if (value.getCode() == ResponseCode.success) {
                                if (value.getReturnValue() != null) {
                                    ...
                                    view.showOneCertInfo(value.getReturnValue());
...
//        params.put("USERCODE", RequestBody.create(MediaType.parse("multipart/form-data"), AccountHelper.getUser().getCODE()));
        ...
        params.put("ATTACHCODE", RequestBody.create(MediaType.parse("multipart/form-data"), certcode));
